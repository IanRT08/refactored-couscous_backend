...
<xsl:stylesheet version="1.0"
  ...
  <!-- URI file:// del logo CENIDET, pasada desde Java; vacío → muestra texto de respaldo -->
  <xsl:param name="logoUri"/>
  <!-- URI file:// de la gráfica del periodo, pasada desde Java; vacío → no se muestra sección -->
  <xsl:param name="graficaUri"/>
      ...
      <fo:page-sequence master-reference="pagina">
        ...
        <fo:static-content flow-name="xsl-region-before">
          <fo:block font-size="8pt" color="#888888" text-align="right" border-bottom="0.3pt solid #cccccc" padding-bottom="3pt">
            AW-SHEF · CENIDET — Reporte de Variables Climáticas
          </fo:block>
        </fo:static-content>
              ...
              <fo:table-row>
                <fo:table-cell display-align="center">
                  <fo:block font-size="15pt" font-weight="bold" color="#003B8E">
                    Reporte de Variables Climáticas
                  </fo:block>
          ...
          <fo:table table-layout="fixed" width="100%" border="0.5pt solid #cccccc"
            ...
            <fo:table-header>
              <fo:table-row background-color="#003B8E">
                ...
                <fo:table-cell padding="4pt"><fo:block color="white" font-weight="bold">Temp (°C)</fo:block></fo:table-cell>
                <fo:table-cell padding="4pt"><fo:block color="white" font-weight="bold">Viento (m/s)</fo:block></fo:table-cell>
                <fo:table-cell padding="4pt"><fo:block color="white" font-weight="bold">Humedad (%)</fo:block></fo:table-cell>
                <fo:table-cell padding="4pt"><fo:block color="white" font-weight="bold">Radiación (W/m²)</fo:block></fo:table-cell>
                <fo:table-cell padding="4pt"><fo:block color="white" font-weight="bold">Presión (hPa)</fo:block></fo:table-cell>
              </fo:table-row>
            </fo:table-header>
            ...
            <fo:table-body>
              <xsl:for-each select="lecturas/lectura">
                <fo:table-row>
                  ...
                  <fo:table-cell padding="3pt"><fo:block><xsl:value-of select="temperatura"/></fo:block></fo:table-cell>
                  <fo:table-cell padding="3pt"><fo:block><xsl:value-of select="viento"/></fo:block></fo:table-cell>
                  <fo:table-cell padding="3pt"><fo:block><xsl:value-of select="humedad"/></fo:block></fo:table-cell>
                  <fo:table-cell padding="3pt"><fo:block><xsl:value-of select="radiacion"/></fo:block></fo:table-cell>
                  <fo:table-cell padding="3pt"><fo:block><xsl:value-of select="presion"/></fo:block></fo:table-cell>
                </fo:table-row>
              </xsl:for-each>
            </fo:table-body>
          </fo:table>
          ...
          <xsl:if test="$graficaUri != ''">
            ...
            <fo:block keep-together.within-page="always">
              <fo:external-graphic content-width="17cm" content-height="20cm" scaling="uniform">
                <xsl:attribute name="src">
                  <xsl:value-of select="$graficaUri"/>
                </xsl:attribute>
              </fo:external-graphic>
            </fo:block>
          </xsl:if>
